...
from .features import model, VECTOR_DIMENSION # Assuming 'model' is exposed for query embedding
...
def episodic_search(query_text, db_connection, top_k=3, include_episodic_context=True):
    ...
    tbl = db_connection.open_table("vault_index")

    direct_hits_df = tbl.search(query_embedding).limit(top_k).to_pandas()
    ...
    if include_episodic_context and not direct_hits_df.empty:
        ...
        for episode_id in unique_episode_ids:
            ...
            episode_items_df = tbl.search().where(f"episode_id = '{episode_id}'").to_pandas()
